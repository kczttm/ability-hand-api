...
def runFingerWave(duration=10, baud=460800):
	# Setup serial connection
	
	if not setupSerial(baud):
...
def no_plot(baud, addr, bufWidth, position, touch):
	global tstart
	global total_count
	global f
	global hand_address
...
def testUARTConnection(ser):
	"""
	Send a simple test signal to verify UART connection
	Args:
...
if __name__ == "__main__":
	...
	# if setupSerial(args.baud):
	# 	printInstructions()
	# 	testUARTConnection(ser)
	# 	ser.close()	
	# 	# conclusion the CP2101 is working.

	
	# plot_lines(args.baud, args.address, args.width, pos, touch)  
	# no_plot(args.baud, args.address, args.width, pos, touch)
	runFingerWave()

	##TODO
	## clearly the communication is not working correctly so dubugging from pin connection
