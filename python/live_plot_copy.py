...
def on_press(key):
    ...
    try:
        ...
        # Special keys (like function keys) will throw AttributeError
        pass
...
def setupSerial(baud):
	...
	try: 
		print("Connecting...")
		ser = serial.Serial(port[0], baud, timeout = 3)
		print("Connected!")
		# print out the error too
...
def calculatePositions(currentPositions, lastCommand):
	...
	if 'esc' in current_keys:
		isFingerWave = False
	elif 'up' in current_keys or 'w' in current_keys:
		isFingerWave = False
		moveUp = True
	elif 'down' in current_keys or 's' in current_keys:
		isFingerWave = False
		moveDown = True
